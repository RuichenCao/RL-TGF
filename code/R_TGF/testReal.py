...
#     data_test_name = ['Crime','HI-II-14','Digg','Enron','Gnutella31','Epinions','Facebook','Youtube','Flickr']
    data_test_name = ['synthetic_1000','synthetic_5000','synthetic_10000','synthetic_50000','synthetic_100000']
    model_file_path = 'models/'
    model_file_ckpt = MODEL_FILE_CKPT
    model_file = model_file_path + model_file_ckpt
    ## save_dir
    save_dir = '../results/R_TGF/real'
    ...
    for j in range(len(data_test_name)):
        ...
        g = nx.read_edgelist(data_test_path+ data_test_name[j] + '.txt', nodetype=int)

        data_test = data_test_path + data_test_name[j] + '.txt'
        solution, time = dqn.EvaluateRealData(model_file, data_test, save_dir, stepRatio)
        df.iloc[0,j] = time
        # print('tri_num',k_triangle(g,solution[:97],1))
        # print('tri_num', k_triangle(g, solution[:163], 1))
        # print('tri_num', k_triangle(g, solution[:228], 1))
        # print('tri_num', k_triangle(g, solution[:326], 1))
        # print('tri_num', k_triangle(g, solution[:424], 1))
        # print(solution)
...
def main():
    model_file_ckpt = 'nrange_30_50_iter_25500.ckpt'

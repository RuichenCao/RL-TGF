...
//    printf ("numCoveredEdges:%d\n", numCoveredEdges);
    ...
    return covered_set.size()==int(0.01*graph->num_nodes);
}
...
//    return -(double)getRemainingCNDScore()/(orig_node_num*orig_node_num*(orig_node_num-1)/2);
    return -double(getRemainingCNDScore());
}
...
//    return CCDScore;
	...
	int nvertices = graph->adj_list.size();	// The number of vertices in the covered_set
